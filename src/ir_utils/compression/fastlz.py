...
def decompress(inf: io.BytesIO) -> bytes:
    r"""Reads a binary file, and returns the decompressed byte string.

    Args:
        inf: A binary file-like object.

    Returns:
        The decompressed byte string.

    Raises:
        ValueError: If the stream ends unexpectedly.

    Examples:
        >>> from io import BytesIO
        >>> data = b"\x08\x01\x02\x03\x04\x05\x06\x07\x08\t"
        >>> decompress(BytesIO(data))
...
def compress(out: io.BytesIO, data: bytes, level: Literal[0, 1, 2, 3] = 2) -> None:  # noqa: C901, PLR0912, PLR0915
    r"""Takes a byte string and populates out with the compressed data.

    Args:
        out: A binary file-like object.
        data: The byte string to compress.
    ...
    Raises:
        ValueError: If the length or distance is invalid.

    Examples:
        >>> from io import BytesIO
        >>> out = BytesIO()
        >>> compress(out=out, data=b"\x01\x02\x03\x04\x05\x06\x07\x08\t", level=3)
        >>> out.getvalue()
        b'\x08\x01\x02\x03\x04\x05\x06\x07\x08\t'
    """
    if level == 0:
